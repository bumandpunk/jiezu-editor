  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error ?? '出现错误，请稍后重试')
      }
    } finally {
      ...
      <Dialog onOpenChange={handleClose} open={open}>
        ...
        >
          ...
          {isDragging && (
            <div className="absolute inset-0 z-50 flex items-center justify-center rounded-lg border-2 border-primary/50 border-dashed bg-primary/5 backdrop-blur-sm transition-all">
              <div className="flex flex-col items-center gap-2 text-primary/70">
                ...
                <p className="font-medium text-sm">在此处拖放图片</p>
              </div>
            </div>
          )}
          ...
          <DialogHeader>
            ...
            <DialogDescription>欢迎告诉我们您的想法</DialogDescription>
          </DialogHeader>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div>
                ...
                <textarea
                  ...
                  placeholder="分享您的想法、建议、功能请求，或反馈问题..."
                  rows={5}
                  value={message}
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  {images.length > 0 ? `${images.length}/${MAX_IMAGES}` : '附件'}
                </button>
                <input
                  accept="image/*"
                  className="hidden"
