    ...
    >
      <PanelSection title="高度">
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5 px-1 pb-1">
          <ActionButton label="低矮 (2.4m)" onClick={() => handleUpdate({ height: 2.4 })} />
          <ActionButton label="标准 (2.5m)" onClick={() => handleUpdate({ height: 2.5 })} />
          <ActionButton label="高挑 (3.0m)" onClick={() => handleUpdate({ height: 3.0 })} />
        </div>
      </PanelSection>
      ...
      <PanelSection title="开洞">
        {node.holes && node.holes.length > 0 ? (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {holeArea.toFixed(2)} m² · {hole.length} 个点
                    </p>
                  </div>
          ...
          <div className="px-2 py-3 text-center text-muted-foreground text-xs">暂无开洞</div>
        )}
